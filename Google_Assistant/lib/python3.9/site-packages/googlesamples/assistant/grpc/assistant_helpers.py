...
END_OF_UTTERANCE = embedded_assistant_pb2.ConverseResponse.END_OF_UTTERANCE     # 말(speak)의 끝

def log_converse_request_without_audio(converse_request):
    """Log ConverseRequest fields without audio data."""
    if logging.getLogger().isEnabledFor(logging.DEBUG):     # 심각도 level메세지가 이 logger에서 처리될지 확인
        resp_copy = embedded_assistant_pb2.ConverseRequest()    # 대화 요청
    ...
    if logging.getLogger().isEnabledFor(logging.DEBUG):
        ...
        if has_audio_data:
            ...
            if resp_copy.audio_out.ListFields():
                logging.debug('ConverseResponse: %s audio_data (%d bytes)', resp_copy, size)
            else:
                logging.debug('ConverseResponse: audio_data (%d bytes)', size)
            return
