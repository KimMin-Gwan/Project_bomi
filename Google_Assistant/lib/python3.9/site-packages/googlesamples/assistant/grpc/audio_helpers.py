...
# translate by JaeHyun Kim  2022.08.26~2022.00.00
...
def normalize_audio_buffer(buf, volume_percentage, sample_width=2):
    ...
    scale = math.pow(2, 1.0*volume_percentage/100)-1    # 오디오 데이터의 볼륨을 조절
...
class ConversationStream(object):
    ...
    Args:
      ...
      sample_width: size of a single sample in bytes.
    """
    def __init__(self, source, sink, iter_size, sample_width):  # initialize
        self._source = source
        self._sink = sink
    ...
    def start_recording(self):
        """Start recording from the audio source. 오디오 소스로 부터 녹음 시작"""
        self._stop_recording.clear()
        self._source.start()
        self._sink.start()

    def stop_recording(self):
        """Stop recording from the audio source. 오디오 소스로 부터 녹음 종료"""
    ...
    def write(self, buf):
        ...
        buf = normalize_audio_buffer(buf, self.volume_percentage)   # 볼륨 조절
